...
void compute_fq_det_recursive_flint(fq_mvpoly_t *result, fq_mvpoly_t **matrix, slong size) {
    ...
    #else
    {
        // Original recursive algorithm (default) - NOW USING UNIFIED_MPOLY
        ...
        // Step 4: Convert fq_mvpoly matrix to unified_mpoly matrix
        timing_info_t convert_start = start_timing();
        DET_PRINT("Converting %ldx%ld matrix to unified format\n", size, size);
        
        for (slong i = 0; i < size; i++) {
            for (slong j = 0; j < size; j++) {
                // Clear the polynomial first
                ...
                for (slong t = 0; t < src_poly->nterms; t++) {
                    ...
                    field_elem_u coeff;
                    if (is_prime_field(ctx)) {
                        // For prime fields, fq_nmod_t is just a wrapper around a single coefficient
                        // Get the coefficient value directly
                        if (fq_nmod_is_zero(src_poly->terms[t].coeff, ctx)) {
                            coeff.nmod = 0;
                        } else {
                            // For prime fields, the coefficient is stored as a degree-0 polynomial
                            // We need to get the constant term
                            nmod_poly_struct *poly = src_poly->terms[t].coeff;
                            if (poly->length > 0) {
                                coeff.nmod = poly->coeffs[0];
                            } else {
                                coeff.nmod = 0;
                            }
                        }
                    } else {
                        // For extension fields, use the conversion function
                        fq_nmod_to_field_elem(&coeff, src_poly->terms[t].coeff, &field_ctx);
                    }
                    
                    // Set the coefficient
                    unified_mpoly_set_coeff_ui(unified_matrix[i][j], &coeff, exp);
                    flint_free(exp);
                }
                
                DET_PRINT("Matrix[%ld][%ld]: %ld terms -> %ld terms\n", 
                         i, j, src_poly->nterms, unified_mpoly_length(unified_matrix[i][j]));
            }
        }
        
        timing_info_t convert_elapsed = end_timing(convert_start);
        print_timing("Matrix conversion to unified format", convert_elapsed);
        ...
        // Step 7: Convert result back to fq_mvpoly
        ...
        if (field_ctx.field_id == FIELD_ID_NMOD || is_prime_field(ctx)) {
            ...
            // Use the existing conversion function or implement inline
            slong nterms = fq_nmod_mpoly_length(fq_poly, fq_ctx);
            DET_PRINT("Converting fq_nmod_mpoly with %ld terms\n", nterms);
            
            if (nterms > 0) {
                // Use existing conversion function if available
        ...
        // Print total timing
        timing_info_t total_elapsed = end_timing(total_start);
        printf("Total computation (unified interface): Wall time: %.6f s, CPU time: %.6f s", 
