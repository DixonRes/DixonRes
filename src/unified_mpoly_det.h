/* unified_mpoly_det.c - Unified polynomial matrix determinant computation */

#include <stdio.h>
#include <stdlib.h>
#include <omp.h>
#include "unified_mpoly_interface.h"  /* Assuming the unified interface header */
...
void compute_unified_mpoly_det_recursive(unified_mpoly_t det_result, 
    ...
    recursion_depth++;
    //printf("DEBUG: Entering determinant computation, size=%ld, depth=%d\n", size, recursion_depth);
    ...
    for (slong col = 0; col < size; col++) {
        ...
        printf("DEBUG: Matrix[0][%ld] is_zero = %d, length = %ld\n", 
               col, is_zero, unified_mpoly_length(mpoly_matrix[0][col]));
    ...
    recursion_depth--;
    printf("DEBUG: Exiting determinant computation, size=%ld, depth=%d\n", size, recursion_depth);
}
...
void compute_unified_mpoly_det_parallel(unified_mpoly_t det_result, 
    ...
    #ifdef _OPENMP
    printf("Parallel computation for %ldx%ld matrix (depth %ld)\n", size, size, depth);
    #endif
