...
/* MDS matrix */
mp_limb_t MDS[3][3] = {{2, 13, 1}, {1, 2, 13}, {13, 1, 2}};
...
int main(void) {
    ...
    for (slong round = 1; round <= RESCUE_R; round++) {
        ...
        for (slong i = 0; i < RESCUE_T; i++) {
            /* 生成左边：z_i^3 */
            char left_side[64];
            snprintf(left_side, sizeof(left_side), "%s^3", var_names[base_z + i]);
            
            /* 生成右边：expression */
            char *right_side = poly_to_string_complete(after_linear[i], var_names, total_vars, ctx);
            
            /* 组合成等式格式：z_i^3 = expression */
            size_t total_len = strlen(left_side) + strlen(right_side) + 4; // " = " + null
            ideal_strings[ideal_count] = (char*) malloc(total_len);
            snprintf(ideal_strings[ideal_count], total_len, "%s = %s", left_side, right_side);
    ...
    if (strcmp(current_poly, "0") != 0) {
        ...
        slong num_roots = our_nmod_poly_roots(roots, univar_poly, 1); 
        ...
        for (slong i = 0; i < roots->num; i++) {
            ...
            roots->roots[i], roots->mult[i]);
      
            /* Verify root */
            mp_limb_t eval = nmod_poly_evaluate_nmod(univar_poly, roots->roots[i]);
            if (eval == 0) {
