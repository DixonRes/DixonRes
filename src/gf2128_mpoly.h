/* gf2128_mpoly.h - GF(2^128) Multivariate Polynomial Array Multiplication and Division */
...
static void gf2128_mpoly_init(gf2128_mpoly_t poly, const gf2128_mpoly_ctx_t ctx) {
    poly->coeffs = NULL;
    poly->exps = NULL;
    poly->length = 0;
    poly->coeffs_alloc = 0;
...
static void gf2128_mpoly_clear(gf2128_mpoly_t poly, const gf2128_mpoly_ctx_t ctx) {
    if (poly->coeffs) flint_free(poly->coeffs);
    if (poly->exps) flint_free(poly->exps);
}

static void gf2128_mpoly_ctx_init(gf2128_mpoly_ctx_t ctx, slong nvars, const ordering_t ord) {
    mpoly_ctx_init(ctx->minfo, nvars, ord);
}

static void gf2128_mpoly_ctx_clear(gf2128_mpoly_ctx_t ctx) {
    mpoly_ctx_clear(ctx->minfo);
}

static void gf2128_mpoly_zero(gf2128_mpoly_t poly, const gf2128_mpoly_ctx_t ctx) {
    poly->length = 0;
}

static void _gf2128_mpoly_fit_length(gf2128_t **coeffs, slong *coeffs_alloc,
...
static void gf2128_mpoly_fit_length_reset_bits(gf2128_mpoly_t poly, slong len, 
...
static void gf2128_mpoly_init3(gf2128_mpoly_t poly, slong alloc, flint_bitcnt_t bits,
...
static void gf2128_mpoly_swap(gf2128_mpoly_t poly1, gf2128_mpoly_t poly2, const gf2128_mpoly_ctx_t ctx) {
    gf2128_mpoly_struct t = *poly1;
    *poly1 = *poly2;
    *poly2 = t;
}

static void _gf2128_mpoly_set_length(gf2128_mpoly_t poly, slong len, const gf2128_mpoly_ctx_t ctx) {
    poly->length = len;
}
...
static void gf2128_dynamic_array_init(gf2128_dynamic_array_t *arr, slong size) {
    arr->size = size;
...
static void gf2128_dynamic_array_clear(gf2128_dynamic_array_t *arr) {
    if (arr->data) {
        free(arr->data);
        arr->data = NULL;
    }
...
#define BLOCK 256  // Smaller block size for GF(2^128) due to larger element size

/* Array multiplication with bounds checking */
static void _gf2128_mpoly_addmul_array1_safe(gf2128_t *poly1, slong array_size,
...
static slong gf2128_mpoly_append_array_LEX_safe(gf2128_mpoly_t P, slong Plen, gf2128_t *coeff_array,
...
static slong gf2128_mpoly_append_array_DEGLEX_safe(gf2128_mpoly_t P, slong Plen, gf2128_t *coeff_array,
...
static slong gf2128_mpoly_append_array_DEGREVLEX_safe(gf2128_mpoly_t P, slong Plen, gf2128_t *coeff_array,
                                                       slong top, slong nvars, slong degb,
                                                       const gf2128_mpoly_ctx_t ctx) {
    slong i;
...
static void _gf2128_mpoly_mul_array_chunked_LEX(
    gf2128_mpoly_t P,
...
static void _gf2128_mpoly_mul_array_chunked_DEG(
    gf2128_mpoly_t P,
    const gf2128_mpoly_t A,
    const gf2128_mpoly_t B,
    ulong degb,
...
static int _gf2128_mpoly_mul_array_LEX(
    gf2128_mpoly_t A,
    const gf2128_mpoly_t B,
    fmpz *maxBfields,
...
static int _gf2128_mpoly_mul_array_DEG(
    gf2128_mpoly_t A,
    const gf2128_mpoly_t B,
    fmpz *maxBfields,
    const gf2128_mpoly_t C,
...
static void gf2128_mpoly_set(gf2128_mpoly_t res, const gf2128_mpoly_t poly, const gf2128_mpoly_ctx_t ctx) {
    if (res == poly) return;
...
static void gf2128_mpoly_set_coeff_ui_ui(gf2128_mpoly_t poly, const gf2128_t *c, 
...
static void gf2128_mpoly_print(const gf2128_mpoly_t poly, const char **vars, 
                               const gf2128_mpoly_ctx_t ctx) {
    if (poly->length == 0) {
        printf("0");
        return;
...
/* Dense polynomial structure for GF(2^128) */
typedef struct {
    gf2128_t *coeffs;
    slong alloc;
    slong *deg_bounds;
...
static void gf2128_mpolyd_init(gf2128_mpolyd_t A, slong nvars) {
    A->coeffs = NULL;
    A->alloc = 0;
...
static void gf2128_mpolyd_clear(gf2128_mpolyd_t A) {
    if (A->coeffs) free(A->coeffs);
    if (A->deg_bounds) free(A->deg_bounds);
}

/* Compute offset for monomial in dense array */
static slong gf2128_mpolyd_offset(const gf2128_mpolyd_t A, const ulong *exp) {
    slong off = 0;
    slong stride = 1;
...
static void gf2128_mpolyd_divrem_univar(gf2128_mpolyd_t Q, gf2128_mpolyd_t R,
...
static int gf2128_mpolyd_is_zero(const gf2128_mpolyd_t A) {
...
static int gf2128_mpoly_is_monomial(const gf2128_mpoly_t poly) {
    return poly->length == 1;
}

/* Get monomial coefficient */
static gf2128_t gf2128_mpoly_get_monomial_coeff(const gf2128_mpoly_t poly) {
    if (poly->length != 1) return gf2128_zero();
    return poly->coeffs[0];
}

/* Get monomial exponent */
static void gf2128_mpoly_get_monomial_exp(ulong *exp, const gf2128_mpoly_t poly, 
                                           const gf2128_mpoly_ctx_t ctx) {
...
static void gf2128_mpoly_to_mpolyd(gf2128_mpolyd_t A, const gf2128_mpoly_t B, 
                                    const gf2128_mpoly_ctx_t ctx) {
    slong nvars = ctx->minfo->nvars;
...
static void gf2128_mpolyd_to_mpoly(gf2128_mpoly_t A, const gf2128_mpolyd_t B,
                                    const gf2128_mpoly_ctx_t ctx) {
    slong nvars = ctx->minfo->nvars;
...
static void gf2128_mpolyd_divrem_multivar(gf2128_mpolyd_t Q, gf2128_mpolyd_t R,
                                          const gf2128_mpolyd_t A, const gf2128_mpolyd_t B,
                                          const gf2128_mpoly_ctx_t ctx) {
...
/* Set degree bounds */
static int gf2128_mpolyd_set_degbounds(gf2128_mpolyd_t A, const slong *bounds) {
    slong size = 1;
...
/* Monomial division */
static int gf2128_mpoly_divides_monomial(gf2128_mpoly_t Q, const gf2128_mpoly_t A, 
...
static void gf2128_mpoly_mul_simple(gf2128_mpoly_t res, const gf2128_mpoly_t a, 
                                    const gf2128_mpoly_t b, const gf2128_mpoly_ctx_t ctx) {
    slong nvars = ctx->minfo->nvars;
...
static int gf2128_mpoly_equal(const gf2128_mpoly_t A, const gf2128_mpoly_t B, 
                              const gf2128_mpoly_ctx_t ctx) {
...
static void gf2128_mpoly_randtest(gf2128_mpoly_t poly, flint_rand_t state,
...
static void gf2128_mpoly_to_fq_nmod_mpoly(fq_nmod_mpoly_t res, const gf2128_mpoly_t poly,
...
static void fq_nmod_mpoly_to_gf2128_mpoly(gf2128_mpoly_t res, const fq_nmod_mpoly_t poly,
                                           const fq_nmod_ctx_t fqctx, const fq_nmod_mpoly_ctx_t fq_mpoly_ctx) {
...
/* Timer function */
static double get_wall_time_128() {
    struct timeval time;
