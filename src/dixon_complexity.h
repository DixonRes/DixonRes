...
// 计算Fuss-Catalan数: (1/(n(d-1)+1)) * C(nd, n)
...
// Optimized variable management with hash table for faster lookups
...
// Optimized lightweight parser state for degree calculation only
typedef struct {
    const char *input;
    size_t pos;
    size_t len;
...
char* dixon_complexity_auto(const char **poly_strings, slong num_polys,
    ...
    // printf("Encoded complexity: %ld (will be added as constant term)\n", complexity_encoded);
    ...
    if (num_remaining == 0) {
        ...
        // Build polynomial: x₁^td + x₂^td + ... + xₖ^td + complexity_encoded
        for (slong i = 0; i < num_remaining; i++) {
            if (i > 0) {
                strcat(eval_poly, " + ");
            }
