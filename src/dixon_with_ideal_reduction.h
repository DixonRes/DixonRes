...
/* 等式信息结构体 */
typedef struct {
    char *main_var_name;      /* 主变量名 */
    slong main_var_degree;    /* 主变量次数 */
    fq_mvpoly_t lhs;         /* 左边表达式 */
    fq_mvpoly_t rhs;         /* 右边表达式 */
    fq_mvpoly_t standard;    /* 标准形式: lhs - rhs */
} equation_info_t;
...
/* 检查字符串是否包含等号 */
int is_equation_format(const char *poly_str) {
    return strchr(poly_str, '=') != NULL;
}

/* 分割等式字符串为左右两部分 */
void split_equation_string(const char *equation_str, char **lhs_str, char **rhs_str) {
...
/* 从左边表达式提取主变量信息 */
void extract_main_variable_from_lhs(const fq_mvpoly_t *lhs, 
                                   char **main_var_name, 
...
/* 解析等式格式的生成元 */
void parse_equation_generator(equation_info_t *eq_info, 
...
char** split_string_r(const char* str, slong* count) {
    ...
    *count = 0;  // Initialize immediately
    ...
    if (len == 0) {
        printf("Empty string, returning NULL\n");
        return NULL;
    }
    ...
    //printf("Allocated result array at: %p\n", (void*)result);
    
    // Verify allocation worked
    for (slong i = 0; i < num_parts; i++) {
        if (result[i] != NULL) {
            printf("ERROR: calloc didn't initialize result[%ld] to NULL!\n", i);
            free(result);
            return NULL;
        }
    }
    ...
    for (slong i = 0; i <= len; i++) {  // Include len to catch final part
        if (i == len || str[i] == ';' || str[i] == ',') {
            ...
            if (part_len > 0) {
                // Bounds check
                if (*count >= num_parts) {
                    printf("ERROR: Exceeded expected parts count\n");
                    break;
                }
                
                result[*count] = (char*) malloc(part_len + 1);
                if (!result[*count]) {
                    printf("ERROR: Failed to allocate part %ld\n", *count);
                    break;
                }
    ...
    //printf("Final count: %ld (expected: %ld)\n", *count, num_parts);
    
    // Verify results don't contain garbage
    for (slong i = 0; i < *count; i++) {
        if (!result[i]) {
            printf("ERROR: result[%ld] is NULL!\n", i);
        } else {
            // Check for obviously corrupt strings
            slong str_len = strlen(result[i]);
            if (str_len > 10000) {  // Reasonable limit
                printf("Warning: result[%ld] suspiciously long (%ld chars)\n", i, str_len);
            }
        }
    }
    
    //printf("=== SAFE split_string_r END ===\n\n");
    return result;
}
...
char* dixon_with_ideal(const char **poly_strings,      // 方程数组
                           slong num_polys,                 // 方程数量
                           const char **elim_vars,          // 消元变量数组
                           slong num_elim_vars,             // 消元变量数量
                           const char *ideal_string,        // 理想字符串（自动提取变量）
                           const fq_nmod_ctx_t ctx) {
...
void test_iterative_elimination_str() {
    printf("\n================================================\n");
    printf("Test: Simplified Iterative Elimination\n");
    printf("================================================\n");
