...
#include <flint/fq_zech_mpoly.h>  // 添加 Zech 支持
#include <flint/fmpz.h>
#include <flint/mpoly.h>
#include <flint/longlong.h>
#include "fq_unified_interface.h"
#include "gf28_mpoly.h"
//#include "gf28_mpoly_sparse.h"
...
typedef struct {
    ...
    union {
        ...
        fq_zech_mpoly_ctx_struct zech_ctx;  // 添加 Zech 上下文
    } ctx;
    
    /* Method selection parameters */
    slong array_size_limit;
    slong dense_threshold;
    ulong zech_size_limit;  // Zech 对数表大小限制（默认 2^20）
...
typedef struct {
    ...
    union {
        ...
        fq_zech_mpoly_struct zech_poly;  // 添加 Zech 多项式
    } data;
    ...
    switch (poly1->field_id) {
        ...
        case FIELD_ID_GF28:
            if (1 || use_gf28_array_mul) {
                ...
                int success = gf28_mpoly_mul_array(C, A, B, native_ctx);//gf28_mpoly_mul_array gf28_mpoly_mul_dense
                
                //printf("end mul\n");
                double end_time = get_wall_time_8();
                //printf("GF(2^8) multiplication time: %.6f seconds\n", end_time - start_time);
                /* TIMING END */
                if (success) {
        ...
        case FIELD_ID_GF2128:
            if (1 || use_gf2128_array_mul) {
                ...
                /* Try array multiplication */
                int success = gf2128_mpoly_mul_array(C, A, B, native_ctx); //gf2128_mpoly_mul_array
