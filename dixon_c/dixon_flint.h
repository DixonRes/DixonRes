 ...
 * 
 * Compile with:  -fsanitize=address -static-libasan 
...
void perform_fq_matrix_row_operations_mvpoly(fq_mvpoly_t ***new_matrix, 
                                                  fq_mvpoly_t ***original_matrix,
                                                  slong nvars, slong npars) {
    slong n = nvars + 1;
    const fq_nmod_ctx_struct *fq_ctx = (*original_matrix)[0][0].ctx;
    ...
    *new_matrix = (fq_mvpoly_t**) flint_malloc(n * sizeof(fq_mvpoly_t*));
    ...
    for (slong i = 0; i < n; i++) {
        ...
        // Initialize ALL elements properly
        for (slong j = 0; j < n; j++) {
            (*new_matrix)[i][j].nvars = 2 * nvars;
            (*new_matrix)[i][j].npars = npars;
            (*new_matrix)[i][j].nterms = 0;
            (*new_matrix)[i][j].alloc = 16;  // Pre-allocate some space
            (*new_matrix)[i][j].terms = (fq_monomial_t*) flint_malloc(16 * sizeof(fq_monomial_t));
            (*new_matrix)[i][j].ctx = fq_ctx;
        }
    }
    ...
    for (slong j = 0; j < n; j++) {
        fq_mvpoly_t *src = &(*original_matrix)[0][j];
        fq_mvpoly_t *dst = &(*new_matrix)[0][j];
        
        // Ensure enough space
        if (src->nterms > dst->alloc) {
            dst->alloc = src->nterms + 8;
            dst->terms = (fq_monomial_t*) flint_realloc(dst->terms, 
                                                        dst->alloc * sizeof(fq_monomial_t));
        }
        
        // Copy terms
        dst->nterms = 0;  // Reset
        for (slong t = 0; t < src->nterms; t++) {
            fq_mvpoly_add_term_fast(dst, src->terms[t].var_exp, 
                                   src->terms[t].par_exp, src->terms[t].coeff);
        }
    }
    ...
    for (slong i = 1; i < n; i++) {
        ...
        for (slong j = 0; j < n; j++) {
            // Compute difference in a temporary polynomial
            fq_mvpoly_t diff;
            diff.nvars = 2 * nvars;
            diff.npars = npars;
            diff.nterms = 0;
            diff.alloc = 16;
            diff.terms = (fq_monomial_t*) flint_malloc(16 * sizeof(fq_monomial_t));
            diff.ctx = fq_ctx;
            
            // diff = M[i][j]
            fq_mvpoly_t *poly_ij = &(*original_matrix)[i][j];
            for (slong t = 0; t < poly_ij->nterms; t++) {
                fq_mvpoly_add_term(&diff, poly_ij->terms[t].var_exp,
                                  poly_ij->terms[t].par_exp, poly_ij->terms[t].coeff);
            }
            
            // diff -= M[i-1][j]
            fq_mvpoly_t *poly_prev = &(*original_matrix)[i-1][j];
            for (slong t = 0; t < poly_prev->nterms; t++) {
                fq_nmod_t neg_coeff;
                fq_nmod_init(neg_coeff, fq_ctx);
                fq_nmod_neg(neg_coeff, poly_prev->terms[t].coeff, fq_ctx);
                
                fq_mvpoly_add_term(&diff, poly_prev->terms[t].var_exp,
                                  poly_prev->terms[t].par_exp, neg_coeff);
                                  
                fq_nmod_clear(neg_coeff, fq_ctx);
            }
            
            // Divide if non-zero
            if (diff.nterms > 0) {
                // Create temporary result
                fq_mvpoly_t quotient;
                quotient.nvars = 2 * nvars;
                quotient.npars = npars;
                quotient.nterms = 0;
                quotient.alloc = 16;
                quotient.terms = (fq_monomial_t*) flint_malloc(16 * sizeof(fq_monomial_t));
                quotient.ctx = fq_ctx;
                
                // Do the division manually (simplified version)
                slong actual_nvars = nvars;
                for (slong t = 0; t < diff.nterms; t++) {
                    slong deg_x = diff.terms[t].var_exp ? diff.terms[t].var_exp[i-1] : 0;
                    slong deg_dual = diff.terms[t].var_exp ? diff.terms[t].var_exp[i-1 + actual_nvars] : 0;
                    
                    // This is a simplified division - in practice use the full algorithm
                    if (deg_x >= deg_dual) {
                        fq_mvpoly_add_term(&quotient, diff.terms[t].var_exp, 
                                          diff.terms[t].par_exp, diff.terms[t].coeff);
                    }
                }
                
                // Copy result to destination
                fq_mvpoly_t *dst = &(*new_matrix)[i][j];
                if (quotient.nterms > dst->alloc) {
                    dst->alloc = quotient.nterms + 8;
                    flint_free(dst->terms);
                    dst->terms = (fq_monomial_t*) flint_malloc(dst->alloc * sizeof(fq_monomial_t));
                }
                
                dst->nterms = 0;
                for (slong t = 0; t < quotient.nterms; t++) {
                    fq_mvpoly_add_term_fast(dst, quotient.terms[t].var_exp,
                                           quotient.terms[t].par_exp, quotient.terms[t].coeff);
                }
                
                // Clear temporary quotient
                for (slong t = 0; t < quotient.nterms; t++) {
                    fq_nmod_clear(quotient.terms[t].coeff, fq_ctx);
                    if (quotient.terms[t].var_exp) flint_free(quotient.terms[t].var_exp);
                    if (quotient.terms[t].par_exp) flint_free(quotient.terms[t].par_exp);
                }
                flint_free(quotient.terms);
            }
            // else: dst remains zero as initialized
            
            // Clear diff
            for (slong t = 0; t < diff.nterms; t++) {
                fq_nmod_clear(diff.terms[t].coeff, fq_ctx);
                if (diff.terms[t].var_exp) flint_free(diff.terms[t].var_exp);
                if (diff.terms[t].par_exp) flint_free(diff.terms[t].par_exp);
            }
            flint_free(diff.terms);
        }
    }
}
...
void find_fq_optimal_maximal_rank_submatrix(fq_mvpoly_t ***full_matrix, 
    ...
    for (slong i = 0; i < nrows; i++) {
        ...
        slong max_terms = 0;
        for (slong j = 0; j < ncols; j++) {
            if (full_matrix[i][j]->nterms > max_terms) {
                max_terms = full_matrix[i][j]->nterms;
            }
        }
        row_degrees[i].degree = max_terms;
    }
    ...
    for (slong j = 0; j < ncols; j++) {
        ...
        col_degrees[j].degree = max_terms;
    }
...
void fq_dixon_resultant(fq_mvpoly_t *result, fq_mvpoly_t *polys, 
    ...
    if (matrix_size > 0) {
        ...
        det_method_t coeff_method = DET_METHOD_INTERPOLATION; // DET_METHOD_RECURSIVE DET_METHOD_KRONECKER DET_METHOD_INTERPOLATION
