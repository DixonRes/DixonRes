...
1. **Robust PML handling**: Build succeeds even when PML library is unavailable (only slightly slower for zero-dimensional systems over prime fields).
...
For the bivariate case, we leverage FLINT's highly optimized resultant algorithms based on *"Optimizations of the Subresultant Algorithm"* [citation], which provide superior performance compared to Dixon matrix methods for two-polynomial systems.
