...
//     printf("    -> Writes reduced resultant to solution+timestamp.dat\n");
    ...
    printf("  Silent mode:\n");
    printf("    %s --silent [--solve] <args>\n", prog_name);
    printf("    -> No console output; solution file is still generated\n");

    printf("FILE FORMAT (Basic Dixon, multiline):\n");
    printf("  Line 1 : field size (prime < 2^63, prime power, or p^k, extension generator defaults to 't')\n");
    printf("  Line 2+: polynomials (comma-separated, may span multiple lines)\n");
    printf("  Last   : variables TO ELIMINATE (comma-separated, #eliminate = #equations - 1)\n");

    printf("FILE FORMAT (Polynomial Solver, multiline):\n");
    printf("  Line 1 : field size (prime < 2^63, prime power, or p^k)\n");
    printf("  Line 2+: polynomials (one per line or comma-separated, n equations in n variables)\n");
...
// Parse a field polynomial string like "t^32 + t^15 + t^9 + t^7 + t^4 + t^3 + 1"
        ...
        if (var_pos == NULL) {
            ...
            if (var_pos > token) {
                ...
                if (strlen(coeff_str) > 0) {
                    coeff = strtoul(coeff_str, NULL, 10);
                } else {
                    coeff = 1;
                }
                free(coeff_str);
            ...
            if (*degree_pos == '^') {
                degree = strtoul(degree_pos + 1, NULL, 10);
            } else {
                degree = 1;
            }
        }
...
static int parse_field_size(const char *field_str, fmpz_t prime, ulong *power, 
    ...
    // Check for colon (custom polynomial specification)
    ...
    // Standard parsing (no changes to existing logic)
...
int main(int argc, char *argv[]) {
    ...
    // Display version information (unless in silent mode, checked later)
    int show_version = 1;
    ...
    if (solve_mode) {
        ...
        if (effective_argc == 2) {
            ...
            if (t) {
                strftime(buffer, sizeof(buffer), "solution_%Y%m%d_%H%M%S.dat", t);
    ...
    if (!parse_field_size(field_str, p_fmpz, &power, &field_poly_str, &gen_var_name)) {
        if (!silent_mode) {
            ...
            fprintf(stderr, "  - With custom polynomial: 2^32: t^32 + t^15 + ...\n");
        }
        if (need_free) {
            free(field_str);
            free(polys_str);
